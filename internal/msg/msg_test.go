...
func TestMessagesAreNotEmpty(t *testing.T) {
	...
	messages := map[string]string{
		...
		"QuitConfirmTitle":      QuitConfirmTitle,
		"QuitConfirmBody":       QuitConfirmBody,
		"ServerRestartedNotice": ServerRestartedNotice,
		"BarItem":               BarItem,
		"BarValue":              BarValue,
		"BarCandidate":          BarCandidate,
		"BarClose":              BarClose,
		"BarExit":               BarExit,
		"BarQuitNow":            BarQuitNow,
		...
		"SetupRegisterTitle":    SetupRegisterTitle,
		"SetupRegisterNotice":   SetupRegisterNotice,
		"SetupRegisterStepName": SetupRegisterStepName,
		...
		"KeyAddConfig":          KeyAddConfig,
		"KeyDoNotAddConfig":     KeyDoNotAddConfig,
		"ConfigFlagUsage":       ConfigFlagUsage,
		"Aborted":               Aborted,
		"ListNameHeader":        ListNameHeader,
		"ListStatusHeader":      ListStatusHeader,
		"ListPathHeader":        ListPathHeader,
		...
		"UpdateAvailable":     UpdateAvailable("v2.0.0"),
		"UpdateArguments":     UpdateArgumentsNotAllowed().Error(),
		"AlreadyLatest":       AlreadyLatest("v2.0.0"),
		"LatestRequest":       LatestReleaseRequestFailed(sample).Error(),
		"LatestStatus":        LatestReleaseStatus("500 Internal Server Error").Error(),
		"LatestDecode":        LatestReleaseDecodeFailed(sample).Error(),
		"GitHubRateLimited":   GitHubRateLimited(10).Error(),
		"InvalidReleaseTag":   InvalidReleaseTag("bad/tag").Error(),
		"AssetMissing":        ReleaseAssetMissing("hso.tar.gz").Error(),
		"AssetURLMissing":     ReleaseAssetURLMissing("hso.tar.gz").Error(),
		"ExecutablePath":      ExecutablePathFailed(sample).Error(),
		"UpdateTemporary":     UpdateTemporaryDirectoryFailed(sample).Error(),
		"DownloadAsset":       DownloadAssetFailed("hso.tar.gz", sample).Error(),
		"DownloadStatus":      DownloadAssetStatus("hso.tar.gz", "404 Not Found").Error(),
		"ReadChecksums":       ReadChecksumsFailed(sample).Error(),
		"ReadArchive":         ReadArchiveFailed("hso.tar.gz", sample).Error(),
		"ChecksumNotFound":    ChecksumNotFound("hso.tar.gz").Error(),
		"CalculateChecksum":   CalculateChecksumFailed("hso.tar.gz", sample).Error(),
		"ChecksumMismatch":    ChecksumMismatch("hso.tar.gz").Error(),
		"ExtractArchive":      ExtractArchiveFailed(sample).Error(),
		"BinaryMissing":       BinaryMissingFromArchive().Error(),
		"PrivilegeAuth":       PrivilegeAuthenticationFailed("sudo", sample).Error(),
		...
		"UnsafePIDDirectory":  UnsafePIDDirectory().Error(),
		"ServerRunning":       ServerRunning(123),
