...
`bin/java` が存在し実行可能か検査する。`release` は列挙に使うが、手動導入 JVM も許すため
起動の必須条件にしない。
...
起動直前にも `bin/java` を検査する。設定後に JVM が消えた場合はスクリプトを実行せず、
設定パスと `hso java change` での再選択を案内する。絶対パスで Java を実行するスクリプトは
上書きしない。別 JVM で失敗すれば次節の検出が案内する。
