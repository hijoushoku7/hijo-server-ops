...
**この警告は stderr へ書かず、ログの経路に流す。** TUI が画面を占有するので、初回起動
では書いた直後に消え、再起動では描画中の画面に割り込んで崩す。`start` が `logs` へ
入れれば、他のサーバー出力と同じ世代・同じ順序で並ぶ（`hso java list` の警告は TUI の
外なので stderr のままでよい）。
...
- process: 最初の子プロセスへの `PATH` 注入、親の `PATH` の維持、`JAVA_HOME` 不変、
  起動直前の JVM 消失時にも継続すること、`Resolve` の 3 経路（設定値をそのまま使用／
  同世代へ再スキャン／注入なし）のどれも起動を妨げないこと
- UI: 現世代だけからの Java 不一致検出、導入済み世代の有無に応じた案内、実 Java が
  不明な場合の案内、自動再起動抑止、狭いモーダルでの折り返し
