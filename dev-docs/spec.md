...
コンソールを通らないので、サーバー自身が出す `Stopping the server` を
ログから拾う（プレイヤー実行時は sendCommandFeedback により
`[名前: Stopping the server]` の形で出る）。どちらも印を付けるだけで、
hso はその場では畳まない。実際にプロセスが終わったときに印を読み、
クラッシュ扱いを外して通常の停止として 3 秒後に hso を終了する。
印は行がキューから溢れても落ちないよう、`offerLog` に渡す前の
読み取り側で拾う。
