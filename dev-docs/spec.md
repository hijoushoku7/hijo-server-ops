...
表示時刻のずれだけは、値を送る形に収まらないので例外にしてある。項目に
`[設定]` を出し、Enter で現在時刻を入れるモーダルを設定モーダルの上に
重ねて開く。地域名（`time.Location`）ではなく「今が何時か」を入れさせ、
システムの時計との差を分で `[ui.time] offset_minutes` に持つ。地域名を
選ばせるには一覧と検索が要るうえ、ユーザーが知りたいのは「手元の時計と
何時間ずれているか」であって地域ではない。

時は 1 時間刻み、分は 00 / 30 のみ。刻みを細かくすると、秒を持たない
入力で分単位を合わせることになり、押すたびに端数が動く。初期値と確定時の
差分は同じ基準（開いた時点のシステムの時計を 30 分に丸めた値）から計算し、
その基準はモーダルが持ち続ける。丸めをどちらか一方だけに掛けると、12:15 の
ような半端な時刻では開いて OK を押すだけで 30 分ずれる。確定時に時計を
読み直す形も同じで、開いている間に丸めの境目（毎時 15 分・45 分）をまたぐと、
指を触れていないのに 30 分動く。入力の意味を「今映っている時刻を何時何分に
直すか」に固定することで、両方とも起きなくなる。オフセットは 30 分刻みで
`(-720, 720]`。日付を
入力させないので、ずれは ±12 時間以内として最も近い日付を採る。UTC+13 等は
表現できない。

加算は表示側だけで行い、保存済みの `time.Time` は書き換えない。書き換えると
変更を重ねるたびにずれが累積する。ログの時刻文字列は折り返し結果に
埋め込んで `lineBuffer` にキャッシュしているので、確定時にキャッシュを
落として直後のフレームで組み直す。落とさないと既存行だけ古い時刻のまま
残る。夏時間の切り替えを挟む区間では、絶対時刻への加算なので表示上のずれが
一定にならないが、手で決め打ちしたオフセットなので許容する。
...
[ui.time]
offset_minutes = 90       # 表示時刻をシステムの時計から何分ずらすか。
                          # 30 分刻み・(-720, 720]。0 のときは書き出さない
